...
/*   Updated: 2024/06/12 10:36:18 by qdo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Bureaucrat.hpp"

Bureaucrat::~Bureaucrat()	{	std::cout << "Bruceaucrat `" << name << "' with grade " << grade << " left" << std::endl;	}

Bureaucrat::Bureaucrat(void) : name("Default"), grade(150) {	std::cout << "Bureaucrat named " << name << " with grade " << grade << " was born" << std::endl;	}
...
{
	o << "Bureaucrat `" << src.getName() << "' with Grade " << src.getGrade();
	return (o);
}
...
void Bureaucrat::signForm(Form & src) const
{
	...
	{
		std::cerr << name << " coudn't sign `" << src.getName() << "' because the Form is already signed" << std::endl;
		return ; 
	}
	if (grade <= src.getGradeToSign())
	{
		src.setSigned();
		std::cout << name << " signed the form `" << src.getName() << "'" << std::endl;
	}
	else
